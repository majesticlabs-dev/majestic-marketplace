...
Commands and scripts read project configuration from `.agents.yml` in the project root. This file is created by `/majestic-engineer:init` during project setup.
...
The toolbox registry enables **automatic stack-specific tool selection** for generic orchestrators like `/majestic-engineer:build-task` and `quality-gate`.
...
| **Design system path** | Path to design system document (generated by `/majestic-engineer:ux-brief`) |
...
**Auto-created:** Running `/majestic-engineer:init` prompts to create local overrides.
