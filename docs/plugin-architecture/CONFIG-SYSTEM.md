...
Commands and scripts read project configuration from `.agents.yml` in the project root. This file is created by `Skill("init")` during project setup.
...
# Planning
plan:
  auto_create_task: true # Auto-create task when Skill("blueprint") completes
...
| `plan.auto_create_task` | Auto-create task when `Skill("blueprint")` completes | `true` \| `false` | `false` |
...
The toolbox registry enables **automatic stack-specific tool selection** for generic orchestrators like `Skill("build-task")` and `quality-gate`.
...
| **Design system path** | Path to design system document (generated by `Skill("ux-brief")`) |
...
**Auto-created:** Running `Skill("init")` prompts to create local overrides.
...
- `Skill("blueprint")` → `docs/plans/<title>.md`
- `Skill("prd")` → `docs/prd/prd-<name>.md`
- `Skill("ux-brief")` → `docs/design/<name>-brief.md`
- `/handoff` → `.agents/handoffs/<timestamp>-<slug>.md`
