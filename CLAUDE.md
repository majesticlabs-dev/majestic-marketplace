...
build_task:
  ...
  coding_styles:                    # Skills to activate during coding
    - majestic-rails:dhh-coder
  design_system_path: docs/design/design-system.md  # UI specifications (generated by /majestic:ux-brief)
  research_hooks:
    - id: gem_research
      mode: auto
...
| **Design system path** | Path to design system document (generated by `/majestic:ux-brief`). Build-task loads this for UI work. |
...
- `toolbox.build_task.design_system_path` → User overrides manifest, falls back to empty (no design system)
...
toolbox:
  build_task:
    ...
    design_system_path: docs/design/design-system.md  # UI specs for build agent
    research_hooks:
      - id: api_docs
        mode: manual
