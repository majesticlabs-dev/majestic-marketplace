...
**Learning:** Claude Code's Bash tool has no TTY access. ANSI escape sequences (like `printf '\033]0;title\007'` for terminal title) are captured as text, not interpreted by the terminal. Neither direct Bash commands nor skill invocations can set terminal window titles. Use Claude Code's `statusLine` feature in `~/.claude/settings.json` instead - it runs in the actual terminal context.
