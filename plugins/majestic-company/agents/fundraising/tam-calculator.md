...
**Formula**: Global Market x Geographic % x Segment % x Category % = TAM
...
| x Problem incidence | % who have the problem |
| x Willingness to pay | % who would pay for solution |
| x ACV | Your annual contract value |
| = TAM | |

**Formula**: TAM = (Customers x Penetration Rate x Annual Revenue per Customer)
...
| x Customers affected | Number with problem |
| = Total problem cost | |
| x Value capture | Typically 10-30% |
| = TAM | |
